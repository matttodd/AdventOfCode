...
def solve(lines):
    ...
    for line in lines:
        line = line.strip()
        cache = {} # key (i, s, e)
        tot += answer(list(line), cache)
    return tot


def answer(code, cache):
    tot = 0
    locs = ['A'] * 26
    for c in code:
        tot += move_to_num(locs, c, cache)
    tag = int("".join(code)[:-1])
    print(tag, tot)
    return tag * tot


def move_to_dir(locs, end, ind, cache):
    # print(locs)
    start = locs[ind]
    s = index_of(start, DIR)
    e = index_of(end, DIR)
    if (ind, start, end) in cache:
        locs[ind] = end
        return cache[(ind, start, end)]
    ...
    for inst in instr:
        tot = 0
        for i in inst:
            tot += move_to_dir(locs, i, ind+1, cache)
        b = min(b, tot)
    cache[(ind, start, end)] = b
    locs[ind] = end
    return b


def move_to_num(locs, end, cache):
    # print(locs)
    start = locs[0]
    ...
    for inst in instr:
        tot = 0
        for i in inst:
            tot += move_to_dir(locs, i, 1, cache)
        b = min(b, tot)
    locs[0] = end
    return b
