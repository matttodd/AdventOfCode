...
def solve():
    ...
    while True:
        try:
            ...
            time_to_end, blizz = turns_to_exit(start, storms, walls, 0, end, mnx, mny, mxx, mxy)
            print(time_to_end)
            time_to_start, blizz = turns_to_exit(end, blizz, walls, time_to_end, start, mnx, mny, mxx, mxy)
            print(time_to_start)
            time_to_end, blizz = turns_to_exit(start, blizz, walls, time_to_start, end, mnx, mny, mxx, mxy)
            return time_to_end
...
def turns_to_exit(curr, blizz, walls, time, end, mnx, mny, mxx, mxy):
    ...
    while queue:
        ...
        for eligible in eligibles:
            if eligible not in walls and eligible not in blizz_set and eligible[1] > -1 and eligible[1] < (mxy + 1):
